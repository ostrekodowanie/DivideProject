...
export default function Budget({ setInfo }) {
    ...
    return (
        <div className={`relative z-20 flex flex-col justify-center ${inputStyles}`}>
            <label htmlFor='budget' className="z-10 absolute inset-0 flex items-center">
                <span className='transition left-6 absolute font-medium text-[#4C3963] duration-300 origin-left -translate-y-[60%] scale-75'>Budget</span>
            </label>
            ...
            {active && <div className="absolute top-full right-0 left-0 rounded-xl overflow-hidden">
                <ul className="flex flex-col">
                    {options.map(option => <li onClick={() => setInput(option)} onFocus={() => setInput(option)} className="bg-background cursor-pointer text-font font-medium py-3 px-5 flex items-center justify-between" key={option}>{option}{input === option && <span className="bg-primary bg-clip-text text-transparent font-bold">X</span>}</li>)}
                </ul>
            </div>}
        </div>
    )
