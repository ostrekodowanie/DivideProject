...
export default function Budget({ setInfo }) {
    ...
    return (
        <div className={`relative z-10 flex flex-col justify-center ${inputStyles}`}>
            ...
            <input className={`bg-transparent select-none outline-none cursor-pointer font-medium transition duration-300 relative z-20 ${active ? 'translate-y-[30%]' : ''}`} readOnly required autoComplete="off" onClick={() => setActive(prev => !prev)} type="text" id='budget' value={input} />
